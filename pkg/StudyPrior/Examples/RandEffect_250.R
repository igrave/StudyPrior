...
mclapply(mc.cores=20, recalc, function(i){
  ...
  save(F.MFP, F.PFP,F.PEP,F.FX0, F.COR, F.PSP, F.SGL, n,x,
       file=paste0("Rand-55/models_f_",i,".rda"))
}
)
...
print("saving")
  save(F.CR9,F.SUM,F.ROB, n, x,
       file=paste0("Rand-55/models_g_",i,".rda"))
})
}
#
...
calc1 <- function(i){
  print(i)
  try({
    load(file=paste0("Rand-55/models_f_",i,".rda"))
    ...
    n.fix <- n
    x.fix <- x
    save(n.fix, x.fix,  bias,  cover, t1e, pow, mse, SIGMAT,  file=paste0("Rand-55/study_Rand_",i,".rda"))
...
TF <- rep(FALSE,500)
for(i in 1:500) TF[i] <-  !file.exists(paste0("Rand-55/study_Rand_",i,".rda"))
recalc <- which(TF)
...
calc <- function(i){
  print(i)
  try({
    load(file=paste0("Rand-55/models_g_",i,".rda"))
    ...
    n.fix <- n
    x.fix <- x
    save(n.fix, x.fix,  bias,  cover, t1e, pow, mse, SIGMAT,  file=paste0("Rand-55/study_grand_",i,".rda"))
...
TF2 <- rep(FALSE,500)
for(i in 1:500) TF2[i] <-  !file.exists(paste0("Rand-55/study_grand_",i,".rda"))
recalc2 <- which(TF2)
