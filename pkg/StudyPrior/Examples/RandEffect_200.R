...
print("saving")
  save(F.CR9,F.SUM,F.ROB, n, x,
       file=paste0("Rand-51/models_g_",i,".rda"))
})
}
#
...
calc1 <- function(i){
  print(i)
  try({
    load(file=paste0("Rand-51/models_f_",i,".rda"))
    ...
    n.fix <- n
    x.fix <- x
    save(n.fix, x.fix,  bias,  cover, t1e, pow, mse, SIGMAT,  file=paste0("Rand-51/study_Rand_",i,".rda"))
...
calc <- function(i){
  print(i)
  try({
    load(file=paste0("Rand-51/models_g_",i,".rda"))
    ...
    n.fix <- n
    x.fix <- x
    save(n.fix, x.fix,  bias,  cover, t1e, pow, mse, SIGMAT,  file=paste0("Rand-51/study_grand_",i,".rda"))
