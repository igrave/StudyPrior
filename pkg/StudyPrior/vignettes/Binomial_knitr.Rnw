...
if(INLA.available) MAP <- binom.MAP.FB(x.multi, n.multi, tau.prior = list(prior= "logtnormal", param=c(0,1)))
@
...
<<PPCOR>>=
PP.FB.COR <- binom.PP.FB.COR(x=x.multi, n=n.multi, d.prior.cor = 0.5)
@
...
plot(MAP.approx,  col="cyan", lwd=2, lty=2, lines.only=TRUE, las=1)
